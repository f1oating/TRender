...
TRenderDevice TRenderDevice::m_TRenderDevice;
...
bool TRenderDevice::Initialize(HWND hwnd, int width, int height) {
	return InitializeDirect3D(hwnd, width, height);
}

void TRenderDevice::BeginFrame(float red, float green, float blue, float alpha) {
	float clearColor[] = { red, green, blue, alpha };
	m_pDeviceContext->ClearRenderTargetView(m_pRenderTargetView.Get(), clearColor);
	m_pDeviceContext->ClearDepthStencilView(m_pDepthStencilView.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
}

void TRenderDevice::EndFrame() {
	m_pSwapChain->Present(1, 0);
}

ID3D11Device* TRenderDevice::GetDevice() const {
    return m_pDevice.Get();
}

ID3D11DeviceContext* TRenderDevice::GetDeviceContext() const {
    return m_pDeviceContext.Get();
}

bool TRenderDevice::OnResize(int width, int height)
{
    HRESULT hr;

    m_pRenderTargetView.Get()->Release();
    m_pDepthStencilView.Get()->Release();
    m_pDepthStencilBuffer.Get()->Release();
...
TRenderDevice* TRenderDevice::GetRenderDevice()
{
    return &m_TRenderDevice;
}

bool TRenderDevice::InitializeDirect3D(HWND hwnd, int width, int height) {
